 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("Max of Empty list is a NosuchelementException"){
    intercept[java.util.NoSuchElementException]{
      max(Nil)
    }
  }

  test("max of single element list is the element itself"){
    assert(max(List(5)) === 5)
  }

  test("sum of single element list is element itself"){
    assert(sum(List(5)) === 5)
  }
